...
<style>
    ...
    .stage-prospecting { background-color: #95a5a6; color: white; padding: 4px 8px; border-radius: 4px; }
    .stage-discovery { background-color: #3498db; color: white; padding: 4px 8px; border-radius: 4px; }
    .stage-solutioning { background-color: #9b59b6; color: white; padding: 4px 8px; border-radius: 4px; }
    .stage-validation { background-color: #e67e22; color: white; padding: 4px 8px; border-radius: 4px; }
    .stage-negotiation { background-color: #f1c40f; color: black; padding: 4px 8px; border-radius: 4px; }
    .stage-closed_won { background-color: #2ecc71; color: white; padding: 4px 8px; border-radius: 4px; }
</style>
...
def get_stage_badge(stage):
    return f'<span class="stage-{stage}">{stage.replace("_", " ").upper()}</span>'
...
# PIPELINE PAGE
    ...
    if df.empty:
        ...
        selected_lead_id = st.selectbox("Select Lead", 
                                       ...
                                       format_func=lambda x: f"{filtered_df[filtered_df['ID']==x]['Company'].values[0]} (ID: {x})")
        
        if selected_lead_id:
            lead_row = filtered_df[filtered_df['ID'] == selected_lead_id].iloc[0]
            
            col1, col2 = st.columns(2)
            
            with col1:
                st.markdown(f"### {lead_row['Company']}")
                st.markdown(f"**Contact:** {lead_row['Contact']}, {lead_row['Title']}")
                if lead_row['Email']:
                    st.markdown(f"**Email:** {lead_row['Email']}")
                if lead_row['LinkedIn']:
                    st.markdown(f"**LinkedIn:** [{lead_row['LinkedIn']}]({lead_row['LinkedIn']})")
                
                st.markdown(f"**Region:** {lead_row['Region']} | **Tier:** {lead_row['Tier']}")
                st.markdown(f"**AUM:** €{lead_row['AUM (M€)']:.0f}M")
            
            with col2:
                st.markdown(f"**Stage:** {lead_row['Stage']}")
                st.markdown(f"**Expected Deal:** €{lead_row['Deal Size (M€)']:.0f}M")
                st.markdown(f"**Expected Yield:** {lead_row['Expected Yield %']:.1f}%")
                
                meddpicc_class = get_meddpicc_class(lead_row['MEDDPICC Score'])
                st.markdown(f"**MEDDPICC Score:** <span class='{meddpicc_class}'>{lead_row['MEDDPICC Score']}/80 ({lead_row['Qualification']})</span>", 
                           unsafe_allow_html=True)
                ...
                new_stage = st.selectbox("Update Stage", 
                                        ...
                                        index=['prospecting', 'discovery', 'solutioning', 
                                               'validation', 'negotiation', 'closed_won', 'closed_lost'].index(lead_row['Stage']))
                
                if st.button("Update Stage"):
                    db.update_lead_stage(selected_lead_id, new_stage)
                    st.success(f"Stage updated to {new_stage}")
                    st.rerun()
            
            if lead_row['Pain Points']:
                st.markdown(f"**Pain Points:** {lead_row['Pain Points']}")
            if lead_row['Use Case']:
                st.markdown(f"**Use Case:** {lead_row['Use Case']}")
...
# MEDDPICC PAGE
    ...
    if df.empty:
        ...
        if selected_lead_id:
            ...
            st.markdown(f"Current Stage: {lead_row['Stage']}")
            ...
            with col2:
                if current_score:
                    ...
                    breakdown_data = {
                        'Element': ['Metrics', 'Economic Buyer', 'Decision Process', 
                                   'Decision Criteria', 'Paper Process', 'Pain', 'Champion', 'Competition'],
                        'Score': [current_score.metrics, current_score.economic_buyer,
                                 current_score.decision_process, current_score.decision_criteria,
                                 current_score.paper_process, current_score.pain,
                                 current_score.champion, current_score.competition]
                    }
...
st.sidebar.markdown("Built for Head of Onchain Solutions")
