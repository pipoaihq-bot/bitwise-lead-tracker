...
<style>
    ...
    * { font-family: 'Inter', sans-serif; }
    ...
    h1 { color: #f8fafc; font-weight: 700; font-size: 2rem; }
    h2 { color: #e2e8f0; font-weight: 600; font-size: 1.5rem; margin-top: 1.5rem; border-bottom: 1px solid #334155; padding-bottom: 0.5rem; }
    section[data-testid="stSidebar"] { background: linear-gradient(180deg, #1e293b 0%, #0f172a 100%); border-right: 1px solid #334155; }
    .metric-card { background: linear-gradient(135deg, #1e293b 0%, #0f172a 100%); border: 1px solid #334155; border-radius: 12px; padding: 1.25rem; text-align: center; }
    .metric-value { font-size: 1.75rem; font-weight: 700; color: #14b8a6; }
    .metric-label { font-size: 0.875rem; color: #94a3b8; margin-top: 0.25rem; }
    .stButton > button { background: #14b8a6; color: white; border: none; border-radius: 8px; padding: 0.6rem 1.2rem; font-weight: 500; }
    .stButton > button:hover { background: #0d9488; }
    .stTabs [data-baseweb="tab-list"] { gap: 0.5rem; }
    .stTabs [data-baseweb="tab"] { background: #1e293b; border-radius: 8px 8px 0 0; color: #94a3b8; border: none; padding: 0.75rem 1rem; }
    .stTabs [aria-selected="true"] { background: #14b8a6 !important; color: white !important; }
</style>
...
# Get leads safely
try:
    leads = db.get_all_leads() if db else []
except:
    leads = []
...
def load_leads_df():
    ...
    for lead in leads:
        try:
            meddpicc = db.get_meddpicc_score(lead.id) if db else None
            data.append({
                'ID': lead.id,
...
if page == "📊 Dashboard":
    ...
    with col3:
        ...
        if task_manager:
            try:
                ...
                pass
        st.markdown(f'<div class="metric-card"><div class="metric-value">{todo}</div><div class="metric-label">Tasks To Do</div></div>', unsafe_allow_html=True)
    with col4:
        p1 = 0
        if task_manager:
            try:
                p1 = len([t for t in task_manager.get_tasks(status='todo') if t.priority.value == 'P1'])
            except:
                pass
        st.markdown(f'<div class="metric-card"><div class="metric-value">{p1}</div><div class="metric-label">P1 Critical</div></div>', unsafe_allow_html=True)
    ...
    if df.empty:
        ...
        with col1:
            ...
            fig = px.pie(region_data, values='Count', names='Region', color_discrete_sequence=colors)
            fig.update_layout(height=300, paper_bgcolor='rgba(0,0,0,0)', font_color='#e2e8f0')
            st.plotly_chart(fig, use_container_width=True)
        
        with col2:
            st.markdown("### MEDDPICC Distribution")
            bins = pd.cut(df['MEDDPICC'], bins=[0, 30, 50, 70, 80], labels=['Low', 'Medium', 'High', 'Qualified'])
    ...
    if df.empty:
        ...
        with st.expander("🔍 Filters", expanded=True):
            ...
            with col2:
                ...
                industry_filter = st.multiselect("Industry", industries) if industries else []
            with col3:
    ...
    if not task_manager:
        ...
        try:
            ...
            st.info("Task stats unavailable")
            ...
            try:
                ...
                for task in tasks:
                    ...
                    with col2:
                        ...
                        st.markdown(f"{priority_colors.get(task.priority.value, '⚪')} **{task.title}**")
                        if task.description:
        ...
        with tab2:
            try:
                ...
                for task in tasks:
                    col1, col2, col3 = st.columns([0.1, 0.7, 0.2])
                    with col1:
                        if st.checkbox("", key=f"ip_{task.id}"):
                            task_manager.update_task_status(task.id, 'done')
                            st.rerun()
                    with col2:
